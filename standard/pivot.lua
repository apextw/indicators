...
function Update(period, mode)
    ...
    if ref == nil then
        ...
        if (source:isAlive()) then
            ...
            t, to = core.getcandle(BS, source:date(source:size() - 1), offset, weekoffset);
        end
    ...
    if (candle < ref:date(0)) then
        if lastRequested ~= nil and candle >= lastRequested then
            return ;
        end
